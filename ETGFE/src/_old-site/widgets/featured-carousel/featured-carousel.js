import('../../plugins/swiper/swiper.js').then(({ default: Swiper }) => {
  Promise.all([
    ...
    import(/* webpackMode: "eager" */ './featured-carousel.scss'),
  ]).then(() => {
    $('.widget.featured-carousel').each(function (i, el) {
      $(el).data('widget', new FeaturedCarousel(el, Swiper))
      $(el).data('widget').init()
    })
  })
})

function FeaturedCarousel (el, Swiper) {
  const self = this
  self.el = $(el)
