...
export default defineComponent({
  ...
  methods: {
    ...
    createHighImage (index) {
      ...
      highImg.setAttribute('src', this.images[index] + '?auto=format&w=1320')
...
<template>
  <div class="package-tile-thumbnail absolute inset-0 group">
    ...
    <button @click="loadPreviousImage" v-if="images.length > 1" type="button" class="rounded-full w-10 h-10 text-primary bg-white absolute top-1/2 -translate-y-1/2 z-20 left-4 transition-all duration-300 hover:bg-body hover:text-white">
      <i class="fas fa-chevron-left"></i>
    </button>
    <button @click="loadNextImage" v-if="images.length > 1" type="button" class="rounded-full w-10 h-10 text-primary bg-white absolute top-1/2 -translate-y-1/2 z-20 right-4 transition-all duration-300 hover:bg-body hover:text-white">
